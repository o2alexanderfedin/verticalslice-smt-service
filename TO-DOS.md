...
## Debug and Fix Stainless Steel Validation Failure - 2025-11-27 10:22

- **Investigate and fix persistent validation failures for thermal expansion constraint** - Stainless steel rod thermal expansion constraint fails validation after 5 attempts with parse error "Expected a RPAREN_TOK, got `at` (SYMBOL)". **Problem:** Request `{"informal_text": "Stainless steel rod with the length of 100' expands less than by 15\" at the temperature 500 Celcius", "enrich": false}` returns HTTP 422. Despite context-rich fixes being implemented, the LLM cannot generate valid SMT-LIB after 5 retries. Error suggests syntax issue with the word "at" being interpreted as a symbol. Need to understand why LLM-generated code has this specific parse error and whether diagnostics provide sufficient context for fixes. **Files:** `src/domain/steps/validation.py:67-235` (retry loop with context-rich fixes), `src/infrastructure/llm/client.py:333-432` (fix_smt_errors_with_context), `src/infrastructure/llm/prompts.py:244-296` (ERROR_FIXING_WITH_CONTEXT_PROMPT), `src/infrastructure/smt/cvc5_executor.py` (SMT execution). **Solution:** (1) Add automated integration test for this specific input to `tests/integration/test_failing_cases.py` (new), (2) Enable debug logging for all pipeline steps to capture: informal_text, formal_text, smt_code at each attempt, solver error messages, LLM fix responses, (3) Run test to collect complete diagnostic trace, (4) Perform root cause analysis: examine if error is in initial extraction or in fix attempts, check if XML context helps LLM understand the issue, verify error parsing extracts correct location, (5) Propose fix: either improve error fixing prompt, adjust XML formatting, add specific handling for measurement/unit constraints, or enhance initial extraction prompt for physical constraints.

## Add Caching to Formalization Step - 2025-11-27 13:08

- **Implement AsyncFileCache for formalization step results** - Formalization step currently lacks caching, unlike enrichment and extraction which both use AsyncFileCache. **Problem:** Observation from production testing shows formalization takes 8.79 seconds with 5 attempts and achieves similarity 0.8573. With `enrich=true`, the enrichment time is negligible (0.0067s likely from cache hit), but formalization always recomputes. For identical informal_text inputs, formalization should be cached to improve performance and reduce API costs. **Files:** `src/domain/steps/formalization.py:1-180` (add cache parameter and caching logic similar to enrichment.py:62-122 and extraction.py:155-183), `src/api/services/pipeline.py` (pass cache instance to FormalizationStep initialization), `src/infrastructure/cache/async_file_cache.py` (existing cache implementation). **Solution:** (1) Add `cache: AsyncFileCache | None = None` parameter to FormalizationStep.__init__(), (2) In execute() method, generate cache key from informal_text and check cache before LLM calls, (3) Store successful formalization results (formal_text, similarity, attempts) in cache after verification passes, (4) Add cache safety comments similar to enrichment.py:108-111 to document exception handling prevents caching errors, (5) Update PipelineService to pass cache instance when initializing FormalizationStep. **Expected benefits:** ~8-9 second reduction in pipeline time for repeated inputs, lower API costs, consistent with existing caching architecture for enrichment and extraction steps.
