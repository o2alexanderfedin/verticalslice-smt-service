...
class Settings(BaseSettings):
    ...
    # API Configuration
    api_title: str = Field(
        default="Semantic-Preserving SMT-LIB Pipeline",
    ...
    formalization_similarity_threshold: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Minimum cosine similarity for formalization step"
    )
    ...
    formalization_max_retries: int = Field(
        ...
        ge=1,
        le=10,
        description="Maximum retries for formalization step"
    )
    extraction_max_retries: int = Field(
        default=5,
        ge=1,
        le=10,
        description="Maximum retries for extraction step"
    )
    validation_max_retries: int = Field(
        default=3,
        ge=1,
        le=10,
        description="Maximum retries for validation step"
    )
    ...
    formalization_temp_start: float = Field(
        ...
        ge=0.0,
        le=2.0,
        description="Starting temperature for formalization"
    )
    ...
    extraction_detail_start: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Starting detail level for extraction"
    )
